...
yahoo.readbydate <-
  function(ticker, start.date, end.date = yahoo.lastday()) {
...
tradingdates <- function(start.date, end.date = yahoo.lastday()) {
  ...
  all.dates <- subset(all.dates,
    as.POSIXlt(all.dates)$wday != 0 &
...
cleanTradingData <-
  function(trading.data, end.date = yahoo.lastday()) {
...
stockCheckErrors <- function(Ticker, ErrThr = 0.10) {
  ...
  err.df <-
    data.frame(Date = stock.df$Date[2:ndays], Ret = stepReturn(stock.df$Adj.Close))
  founderrors <-
    err.df[which(err.df$Ret > ErrThr | err.df$Ret < (-ErrThr)), ]
  return(founderrors)
}
...
stockGetdata <- function(vticker) {
  ...
  if (file.exists(filename))
  {
    ...
    if (file.exists(basefilename)) {
      ...
      asset.newdata <-
        read.csv(basefilename,
          colClasses = yahoo.colClasses(),
          na.strings = 'null')
